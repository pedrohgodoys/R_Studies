...
cv <- lapply(
  ...
  FUN = function(x) {
    ...
    classifier <- svm(
      formula = Purchased ~ .,
      data = trainFold, # train classifier on each one of the training folds
      type = "C-classification",
      kernel = "radial"
    )

    yPred <- predict(object = classifier, newdata = testFold[, -3])

    cm <- table(testFold[, 3], yPred)
